...
class DrumInstrument:
    ...
    _registry: dict[str, "DrumInstrument"] = (
        {}
...
class KeymapLoader:
    ...
    @classmethod
    def get_midi_note(
        cls, instrument_name: str, keymap_name: str
    ) -> int | None:
    ...
    def get_unmapped_instruments(cls, keymap_name: str) -> set[str]:
        ...
        mapped = {
            name
            for name, data in keymap.get("instruments", {}).items()
    ...
    def generate_user_keymap(cls, target_path: Path) -> None:
        ...
        template = cls.get_keymap("template")
        if template is None:
            raise FileNotFoundError(
                "No template keymap found in mappings directory."
            )
...
class DrumKit:
    ...
    def get_midi_note(
        self, instrument_name: str, keymap_name: str = "gm"
    ) -> int | None:
    ...
    def get_velocity_range(self, instrument_type: str) -> VelocityRange:
        ...
        return self.velocity_ranges.get(
            instrument_type, VelocityRange()  # Default range (1-127)
        )

    def randomize_velocity(
        self, instrument_type: str, base_velocity: int | None = None
    ) -> int:
    ...
    def from_preset(cls, preset_name: str) -> "DrumKit":
        ...
        The preset_name must match an existing .json file stem in the mappings
        directory. No aliases, no fallbacks.
        """
        keymap = KeymapLoader.get_keymap(preset_name)
        if keymap is None:
            raise ValueError(
                f"Unknown mapping '{preset_name}'. Must be a JSON file stem "
                f"in mappings/ (e.g. 'template', 'gm', 'ad2', 'ezd3', 'xg')."
            )
